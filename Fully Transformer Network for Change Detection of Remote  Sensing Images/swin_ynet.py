...
class SwinTransDecoder(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=2,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer ==0 :
                layer_up = PatchExpand(input_resolution=(self.patches_resolution[0] // (2 ** (self.num_layers-1-i_layer)),
                self.patches_resolution[1] // (2 ** (self.num_layers-1-i_layer))), dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)), dim_scale=2, norm_layer=norm_layer)
            else:
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(img_size//patch_size,img_size//patch_size),dim_scale=4,dim=embed_dim,patchsize=patch_size)
        ...
        self.ppattention.append(ppattention(1024))
        self.ppattention.append(ppattention(512))
        self.ppattention.append(ppattention(256))
        self.ppattention.append(ppattention(128))
...
class encoder1(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = SwinTransEncoder(img_size=224,patch_size=4,in_chans=3,num_classes=2,embed_dim=128,depths=[2,2,18,2],depths_decoder=[4,4,4,4],num_heads=[4,8,16,32],window_size=7)
    ...
    def load_from(self):
        ...
        if pretrained_path is not None:
            ...
            if "model"  not in pretrained_dict:
                ...
                #msg = self.encoder2.load_state_dict(pretrained_dict,strict=False)
                # print(msg)
                return
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        #self.encoder2 = SwinTransEncoder(img_size=384,patch_size=4,in_chans=3,num_class=2,embed_dim=128,depths=[2,2,18,2],num_heads=[4,8,16,32],window_size=7)
        self.decoder = SwinTransDecoder()


    def forward(self,img1,img2):
        x,x_downsample1,x_downsample2 = self.encoder1(img1,img2)
        #@x2,x_downsample2 = self.encoder1(img2)
        #print(x1.size(),x_downsample2[0].size(),x_downsample2[1].size(),x_downsample2[2].size(),x_downsample2[3].size())
        #print(sum(x1-x_downsample1[3]))
